...
const PackageDetail = () => {
    ...
    const chartData = useMemo(() => {
        if (!pkg || !pkg.itinerary || pkg.itinerary.every(d => !d.elevation)) return [];
        return pkg.itinerary.map(day => ({
            name: `Day ${day.day}`,
            elevation: day.elevation || 0,
            camp: day.accommodation.split(' (')[0]
        }));
    }, [pkg]);

    useEffect(() => {
        window.scrollTo(0, 0);
        // Find the specific package based on URL params
        const foundPkg = packagesData.find(
            p => p.routeId === routeId && p.id === packageId
        );
        ...
        if (foundPkg) {
            ...
            navigate('/trekking/kilimanjaro');
        }
    }, [routeId, packageId, navigate]);

    if (!pkg) return null; // or a sophisticated loading skeleton
